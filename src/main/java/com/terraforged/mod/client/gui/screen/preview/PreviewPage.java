...
public final class PreviewPage implements Page {
    ...
    public Component title() {
        return Component.translatable("newterraforged.gui.preview");
    }
    ...
    public void init(ConfigScreen screen, int left, int top, int width, int height) {
        ...
        this.tipButtons.add(screen.addRenderableWidget(new HoverTipButton(
                x0, top, btnW, btnH,
                Component.translatable("newterraforged.gui.preview.seed"),
                b -> {
        ...
        this.tipButtons.add(screen.addRenderableWidget(new HoverTipButton(
                x1, top, btnW, btnH,
                Component.literal(shortMode(this.preview.previewSettings().display)),
                b -> {
                    this.preview.previewSettings().display = this.preview.previewSettings().display.next();
                    b.setMessage(Component.literal(shortMode(this.preview.previewSettings().display)));
                    this.refresh();
                },
                () -> Component.literal(fullMode(this.preview.previewSettings().display)))));

        this.tipButtons.add(screen.addRenderableWidget(new HoverTipButton(
                x2, top, btnW, btnH,
                Component.literal(shortFilter(this.preview.previewSettings().terrainFilter)),
                b -> {
                    this.preview.previewSettings().terrainFilter = nextTerrainFilter(this.preview.previewSettings().terrainFilter);
                    b.setMessage(Component.literal(shortFilter(this.preview.previewSettings().terrainFilter)));
                    this.refresh();
                },
                () -> Component.literal(fullFilter(this.preview.previewSettings().terrainFilter)))));

        this.tipButtons.add(screen.addRenderableWidget(new HoverTipButton(
                x3, top, btnW, btnH, Component.literal("Zoom -"),
                b -> {
        ...
        this.tipButtons.add(screen.addRenderableWidget(new HoverTipButton(
                x4, top, lastW, btnH, Component.literal("Zoom +"),
                b -> {
        ...
        if (corrGate) {
            ...
            this.tipButtons.add(screen.addRenderableWidget(new HoverTipButton(
                    ...
                    Component.literal(on ? "Corridors: On" : "Corridors: Off"),
                    b -> {
                        this.preview.previewSettings().showCorridorDirections =
                                !this.preview.previewSettings().showCorridorDirections;
                        boolean next = this.preview.previewSettings().showCorridorDirections;
                        b.setMessage(Component.literal(next ? "Corridors: On" : "Corridors: Off"));
                        this.refresh();
                    },
                    () -> Component.literal("Toggle directed corridor overlay (A→B arrows)"))));
        }
