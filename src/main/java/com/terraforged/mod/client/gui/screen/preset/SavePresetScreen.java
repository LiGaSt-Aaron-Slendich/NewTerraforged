...
public final class SavePresetScreen extends Screen {
    ...
    public SavePresetScreen(ConfigScreen parent, SettingsDraft draft) {
        super(Component.translatable("newterraforged.gui.presets.save.title"));
        this.parent = parent;
        this.draft = draft;
    }
    ...
    protected void init() {
        ...
        this.nameBox = new EditBox(this.font, cx - 100, cy - 10, 200, 20, Component.translatable("newterraforged.gui.presets.save.name"));
    ...
    private void save() {
        try {
            ...
            this.minecraft.getToasts().addToast(SystemToast.multiline(
                    ...
                    Component.literal("NewTF settings saved"),
                    Component.literal(path.toString())
            ));
            this.onClose();
        } catch (Throwable t) {
            TerraForged.LOG.error("Failed to save preset", t);
            this.minecraft.gui.getChat().addMessage(Component.literal("NewTF: failed to save preset"));
        }
    }
    ...
    public void render(PoseStack pose, int mouseX, int mouseY, float partialTick) {
        ...
        drawCenteredString(pose, this.font, Component.translatable("newterraforged.gui.presets.save.hint", PresetLibrary.userDir().toString()), this.width / 2, 56, 0xA0A0A0);
