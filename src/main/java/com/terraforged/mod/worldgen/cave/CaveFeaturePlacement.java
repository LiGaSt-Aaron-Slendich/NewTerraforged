...
public final class CaveFeaturePlacement {
    ...
    public static boolean mayPlace(Holder<PlacedFeature> feature, CaveFeatureRules.Anchor anchor, BlockPos airAnchor, ChunkAccess chunk) {
        if (CaveFeatureClassifier.isTree(feature)) {
            return false;
        }
        if (CaveFeatureClassifier.classify(feature) == CaveFeatureMass.BLOCKED) {
            return false;
        }
    ...
    public static int massPriority(CaveFeatureMass mass) {
        return switch (mass) {
            case LARGE -> 0;
            case MEDIUM -> 1;
            case SMALL -> 2;
