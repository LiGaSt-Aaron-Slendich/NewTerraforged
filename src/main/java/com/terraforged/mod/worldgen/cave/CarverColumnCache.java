...
final class CarverColumnCache {
    ...
    /** Max excess of terrain-data height over heightmap тАФ river/lake banks inflate far higher. */
    ...
    private boolean isRiverCorridorColumn(ChunkAccess chunk, int dx, int dz, int sea) {
        ...
        // Always sample live riverNoise тАФ do not trust region/chunk proximity caches alone.
        ...
        if (this.cachedTerrain != null) {
            ...
            // TerrainData river: ~0 channel тАж ~1 dry.
    ...
    /** Full-column synapse scan тАФ sparse chunk gate + border columns only in mega/giga chunks. */
     ...
     * and border columns eligible тАФ prevents vertical chunk-face walls in continuous synapse networks.
     */
    ...
    /** Cheap slope estimate from cached heightmap тАФ avoids terrain gradient sampling per column. */
    ...
    /**
     * Sparse probe тАФ early exit on first eligible column; fills remaining columns only when probe misses.
     */
