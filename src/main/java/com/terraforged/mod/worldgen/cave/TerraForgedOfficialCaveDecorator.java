...
public final class TerraForgedOfficialCaveDecorator {
        ...
        for (int lx = 2; lx < 16; lx += 4) {
            for (int lz = 2; lz < 16; lz += 4) {
                ...
                if (floorPlaced < 4 && random.nextFloat() < 0.55f) {
                    ...
                    if (FeaturePlacement.place(pointed, region, (ChunkGenerator)generator, random, floorAnchor, true)) {
                        ++floorPlaced;
                    }
                }
                if (ceilPlaced >= 4) {
                ...
                if (FeaturePlacement.place(pointed, region, (ChunkGenerator)generator, random, new BlockPos(wx, ceilY, wz), true)) {
                    ++ceilPlaced;
                }
            }
        }
    ...
    private static Map<Holder<Biome>, BlockPos> filterByPaintedVolume(ChunkAccess chunk, Map<Holder<Biome>, BlockPos> painted) {
        ...
        for (Map.Entry<Holder<Biome>, Integer> entry : quartCounts.entrySet()) {
            Holder<Biome> biome = entry.getKey();
            if (entry.getValue() < 1 || filtered.containsKey(biome) || CaveBiomeIds.isBlockedCaveBiome(biome)) {
                continue;
            }
            BlockPos seed = TerraForgedOfficialCaveDecorator.findPaintedSeed(chunk, biome, minY, maxY);
            if (seed != null) {
                filtered.put(biome, seed);
            }
        }
        return filtered.isEmpty() ? painted : filtered;
    }

    private static BlockPos findPaintedSeed(ChunkAccess chunk, Holder<Biome> biome, int minY, int maxY) {
        int chunkX = chunk.getPos().getMinBlockX();
        int chunkZ = chunk.getPos().getMinBlockZ();
        for (int lx = 0; lx < 16; ++lx) {
            for (int lz = 0; lz < 16; ++lz) {
                for (int y = minY; y <= maxY; y += 4) {
                    ...
                    if (painted == null || !CaveBiomeIds.sameBiomeKey(painted, biome)) {
                        continue;
                    }
                    int floorY = TerraForgedOfficialCaveDecorator.findSimpleFloorAir(chunk, lx, lz, minY, maxY);
                    if (floorY >= 0) {
                        return new BlockPos(chunkX + lx, floorY, chunkZ + lz);
                    }
                    return new BlockPos(chunkX + lx, y, chunkZ + lz);
                }
            }
        }
        return null;
    ...
    private static void decorateFloorPass(BlockPos pos, WorldGenLevel region, Generator generator, CarverChunk carver, BiomeGenerationSettings settings, WorldgenRandom random, ChunkAccess chunk, Holder<Biome> biome, int chamberSpan, boolean megaGigaZone, Boolean largeOnly) {
        ...
        for (int stageIndex = firstStage; stageIndex <= lastStage; ++stageIndex) {
            ...
            for (int featureIndex = 0; featureIndex < stage.size(); ++featureIndex) {
                ...
                FeaturePlacement.place(placed, region, (ChunkGenerator)generator, random, placePos, true);
            }
        }
    }
    ...
    private static void decorateCeiling(BlockPos airPos, WorldGenLevel region, Generator generator, BiomeGenerationSettings settings, WorldgenRandom random, ChunkAccess chunk, Holder<Biome> biome) {
        ...
        for (int stageIndex = firstStage; stageIndex <= lastStage; ++stageIndex) {
            ...
            for (int featureIndex = 0; featureIndex < stage.size(); ++featureIndex) {
                ...
                random.setFeatureSeed(baseSeed, featureIndex, stageIndex);
                if (FeaturePlacement.place(placedFeature, region, (ChunkGenerator)generator, random, placePos, true) && ++placed >= ceilingBudget) {
                    return;
                }
            }
        }
    ...
    public static boolean shouldSkipForeignFeaturePublic(Holder<PlacedFeature> placed, Holder<Biome> biome) {
        ...
        ResourceLocation featureId = CaveFeatureClassifier.featurePath(placed);
    ...
    private static boolean isBlockedFeaturePath(String path, Holder<Biome> biome) {
        ...
        if (lower.contains("geode") || lower.contains("mega_geode") || lower.contains("crystal_geode")
                ...
                || CaveFeatureClassifier.isTree(lower)) {
            return true;
        }
    ...
    private static boolean shouldSkipChamberFeature(Holder<PlacedFeature> placed, Holder<Biome> biome, int chamberSpan, boolean nearSurfaceCrust) {
        ...
        boolean giant = path.contains("giant") || path.contains("huge_") || CaveFeatureClassifier.isCaveFloorLarge(placed);
