...
import com.terraforged.mod.worldgen.cave.CaveFeatureMass;
...
public final class CaveFeaturePlan {
    ...
    public static CaveFeaturePlan build(BiomeGenerationSettings settings, boolean modCaveBiome) {
        ...
        for (int stageIndex = 0; stageIndex < stages.size(); ++stageIndex) {
            ...
            for (int featureIndex = 0; featureIndex < stage.size(); ++featureIndex) {
                ...
                CaveFeatureMass mass;
                Holder placed = stage.get(featureIndex);
                if (!CaveFeatureFilters.isAllowedFeature((Holder<PlacedFeature>)placed, stageIndex) || CaveFeatureClassifier.isTree((Holder<PlacedFeature>)placed) || CaveFeatureClassifier.spawnsSurfaceVegetation((Holder<PlacedFeature>)placed) || (mass = CaveFeatureClassifier.classify((Holder<PlacedFeature>)placed)) == CaveFeatureMass.BLOCKED) continue;
                StageFeature entry = new StageFeature(stageIndex, featureIndex, (Holder<PlacedFeature>)placed, mass, topLayer);
                boolean ceilingFeature = CaveFeatureClassifier.isCaveCeilingFeature((Holder<PlacedFeature>)placed) || CaveFeatureClassifier.isCeilingScatter((Holder<PlacedFeature>)placed);
                boolean bl = floorFeature = CaveFeatureClassifier.isCaveFloorLarge((Holder<PlacedFeature>)placed) || !ceilingFeature || CaveFeatureClassifier.isDualSurfaceFeature((Holder<PlacedFeature>)placed);
                if (floorFeature) {
                    floor.add(entry);
                }
    ...
    private static boolean isAnchorFeature(Holder<PlacedFeature> placed, CaveFeatureMass mass) {
        if (CaveFeatureFilters.isAnchorOnlyFeature(placed)) {
            return true;
        }
        if (CaveFeatureClassifier.isCaveFloorLarge(placed) || CaveFeatureClassifier.isCaveCeilingFeature(placed)) {
            return true;
        }
        return mass == CaveFeatureMass.LARGE;
    }

    public record StageFeature(int stageIndex, int featureIndex, Holder<PlacedFeature> feature, CaveFeatureMass mass, boolean topLayer) {
    }
